 ...
 * is found, call pointelle_sslider_display_the_image() to format it.
 *
 * @since 0.1
 ...
 */
function pointelle_sslider_get_the_image( $args = array() ) {
	global $post;
	$post_id = $post->ID;
	...
	$args = apply_filters( 'pointelle_sslider_get_the_image_args', $args );
 ...
 * @return array|bool
 */
function pointelle_sslider_image_by_custom_field( $args = array() ) {
 ...
 * other filters are later added in the pointelle_sslider_display_the_image() function.
 *
 * @since 0.4
 * @param array $args
 * @return array|bool
 */
function pointelle_sslider_image_by_the_post_thumbnail( $args = array() ) {
 ...
 * @return array|bool
 */
function pointelle_sslider_image_by_attachment( $args = array() ) {
...
/**
 * Scans the post for images within the content.  Not called by default with pointelle_sslider_get_the_image().
 ...
 * @return array|bool
 */
function pointelle_sslider_image_by_scan( $args = array() ) {
 ...
 * given in an array.  Not used with pointelle_sslider_get_the_image() by default.
 *
 * @since 0.3
 * @param array $args
 * @return array
 */
function pointelle_sslider_image_by_default( $args = array() ) {
 ...
 */
function pointelle_sslider_display_the_image( $args = array(), $image = false ) {
	...
	if($timthumb=='1'){
	  $html = '<img src="'.pointelle_slider_plugin_url( 'includes/timthumb.php' ) .'?src='. $image['url'] . '&amp;h='.$h.'&amp;w='.$w.'" alt="' . esc_attr( strip_tags( $image_alt ) ) . '" class="' . esc_attr( $class ) . '"' . $width . $height . $style .' />';	  
    }
	else{
	...
	if ( $link_to_post ) {
		$html = '<a class="pointelle_thumb_anchor" href="' . get_permalink( $post_id ) . '" title="' . esc_attr( apply_filters( 'the_title', get_post_field( 'post_title', $post_id ) ) ) . '">' . $html . '</a>';}
	else {
	    if($permalink!='') {
	      $html = '<a class="pointelle_thumb_anchor" href="' . $permalink . '" title="' . esc_attr( apply_filters( 'the_title', get_post_field( 'post_title', $post_id ) ) ) . '">' . $html . '</a>';
		}
	}
...
/**
 * Get the image with a link to the post.  Use pointelle_sslider_get_the_image() instead.
 *
 * @since 0.1
 * @deprecated 0.3
 */
function pointelle_sslider_get_the_image_link( $deprecated = '', $deprecated_2 = '', $deprecated_3 = '' ) {
	pointelle_sslider_get_the_image();
}
