...
{                                     
	...
	{                                    
		...
		if( $error )
			array_push( $li_class, 'icon', 'error' );
		...
		switch( $field['type'] )
		{
			...
			case 'radio':
				...
				foreach( $field['option'] as $i => $option )
				{
					...
					$html .= "\t\t\t\t\t\t<input type=\"radio\" name=\"yiw_contact[{$field[data_name]}]\" id=\"{$field[data_name]}-$form-$i\" value=\"$option\"$selected /><label style=\"display:inline;cursor:pointer;\" for=\"{$field[data_name]}-$form-$i\"> $option</label><br />\n";
				}
			break;
