...
class contact_info extends WP_Widget
{
    function contact_info() 
    {
		$widget_ops = array( 
            ...
            'description' => __( 'Widget with a simple contact information.', TEXTDOMAIN )
        );

		$control_ops = array( 'id_base' => 'contact-info' );

		$this->WP_Widget( 'contact-info', __( 'Contact Info', TEXTDOMAIN ), $widget_ops, $control_ops );   
	}
	...
	function widget( $args, $instance )
	{
		...
		$text .= '  <ul>';
		if ( ! empty( $instance['phone'] ) ) $text .= '    <li class="phone-icon"><span>' . $instance['phone'] . '</span></li>';
		if ( ! empty( $instance['fax'] ) )   $text .= '    <li class="fax-icon"><span>'   . $instance['fax']   . '</span></li>';
		$text .= '  </ul>';
		$text .= '</div>'; 
